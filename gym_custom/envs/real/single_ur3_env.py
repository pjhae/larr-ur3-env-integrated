...
class SingleUR3RealEnv(gym_custom.Env):
    ...
    def set_initial_joint_pos(self, q=None):
        if q is None: pass
            ...
            self._init_qpos = np.concatenate([self.interface_right.get_joint_positions()]).ravel()
        else:
            assert q.shape[0] == 6
            self._init_qpos = q
    ...
    def set_initial_joint_vel(self, qd=None):
        if qd is None: pass
            ...
            self._init_qvel = np.concatenate([self.interface_right.get_joint_speeds()]).ravel()
        else:
            assert qd.shape[0] == 6
            self._init_qvel = qd
    ...
    def set_initial_gripper_pos(self, g=None):
        if g is None: pass
            ...
            self._init_gripperpos = np.concatenate([self.interface_right.get_gripper_position()]).ravel()
        else:
            assert g.shape[0] == 1
    ...
    def set_initial_gripper_vel(self, gd=None):
        if gd is None: pass
            ...
            self._init_gripperpos = np.concatenate([self.interface_right.get_gripper_speed()]).ravel()
        else:
            assert gd.shape[0] == 1
...
def servoj_speedj_example(host_ip_right, rate):
    ...
    if prompt_yes_or_no('current qpos is \r\n right: %s deg?\r\n'
        %(np.rad2deg(real_env._init_qpos[:6]))) is False:
        print('exiting program!')
        sys.exit()
    ...
    if prompt_yes_or_no('servoj to \r\n right: %s deg?\r\n'
        %(np.rad2deg(goal_qpos_right))) is False:
        print('exiting program!')
        sys.exit()
