...
def fetch_pickandplace_random_action():
    env = gym_custom.make('FetchPickAndPlace-v1')
    obs = env.reset()
    dt = env.dt
...
def gripper_grasp_test():
    env = gym_custom.make('FetchGripper-v0')
    obs = env.reset()
    dt = env.dt

    PID_gains = {'P': 100, 'I': 50, 'D': 20}
    ...
    for t in range(int(5/dt)):
        pole_xyz = np.array([0.0, 0.0, -0.03])
        gripper_pos = 10
...
if __name__ == '__main__':
    # fetch_pickandplace_random_action()
    gripper_grasp_test()